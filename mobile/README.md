...
Service ID — deliberate scope decision).
...
│   ├── theme/              # color palette (ported from the iOS app), Material 3, shared components
...
└── features/               # one feature = one {application,data,presentation} folder
    ...
    └── documents/              checklist/    settings/    premium/
```

Architecture rules: every feature has `application/`, `data/`,
...
Rewrite shipped (1.0.0): 12 feature modules (10 ported from the original
SwiftUI app + premium & onboarding), 499 unit/widget tests green, mocked
E2E smoke green on iOS and Android, icons and splash generated, web/PWA
build live.
